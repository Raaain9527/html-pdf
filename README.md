...
| 优先级 | 功能 | 描述 |
|------|------|------|
| **P0** | 实时预览 | 导出前渲染预览，视口宽度/缩放等参数变化时预览实时更新。左右分栏布局，解决"盲操作"问题 |
| **P0** | 交互式导出 | 预览中开启交互模式后，可点击页面元素（展开折叠区、切换标签等），导出交互后的 DOM 状态 |
| **P0** | 多文件上传 | 桌面端和 Web 端一次选择多个 HTML 文件批量导出，PDF 默认以原文件名命名 |
